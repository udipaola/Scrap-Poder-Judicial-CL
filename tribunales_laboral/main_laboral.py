...
def main():
    ...
    parser.add_argument('--debug', action='store_true', help='Activar logs detallados para depuración')
    ...
    while True:
        ...
        try:
            tasks_para_pool = [(task, lock, args.headless, stop_event, args.debug) for task in tareas_pendientes]
            results_async = []
            ...
            for res in results_async:
                try:
                    ...
                    if isinstance(resultado, str) and (resultado.startswith('IP_BLOCKED') or resultado.startswith('ERROR')):
                        ...
                        print(f"¡SEÑAL DE RETRY DETECTADA ({resultado})! Activando evento de parada...")
            ...
            if ip_bloqueada_detectada or retry_detectado:
                ...
                if ip_bloqueada_detectada:
                    ...
                    print("\nElementClickInterceptedException detectada. Reiniciando el ciclo de procesamiento sin rotación de IP...")
                continue
            else:
